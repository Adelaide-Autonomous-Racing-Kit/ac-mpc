...
class TrackLimitPerception:
    ...
    def _process_track_points(self, tracks: Dict):
        ...
        if self._use_interpolated_centreline:
            ...
            left_track = smooth_track_with_polyfit(left_track, num_points)
            right_track = smooth_track_with_polyfit(right_track, num_points)
            ...
            centre_track = [origin_points, centre_track]
            centre_track = np.concatenate(centre_track, axis=0)
            centre_track = smooth_track_with_polyfit(centre_track, num_points)
