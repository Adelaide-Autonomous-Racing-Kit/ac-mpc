...
class Perceiver:
    ...
    def perceive(self, obs: List):
        ...
        self._maybe_add_track_limits(obs)
        return obs

    def _preprocess_observations(self, obs: List) -> Dict:
        output_obs = ObservationDict(obs)
        output_obs["CameraFrontRGB"] = self._encode_decode_image(output_obs["CameraFrontRGB"])
    ...
    def _encode_decode_image(self, image:np.array) -> np.array:
        ...
        image = np.asarray(Image.open(
            io.BytesIO(
                TURBO_JPEG.encode(image, pixel_format=TJPF_BGRX)
            )
        ))
        return image
    ...
    def _assert_image_size_is_correct(self, obs: Dict):
        if not self._is_image_size_correct(obs["CameraFrontRGB"]):
            ...
            message = f"Resizing image from {original_size[0]}x{original_size[1]} "
    ...
    def _maybe_add_track_limits(self, obs: Dict):
...
class PerceptionProcess(mp.Process):
    ...
    def __setup_segmentor(self, cfg: Dict):
        self._segmentor = TrackSegmenter(cfg)

    def __setup_track_extractor(self, cfg: Dict):
        self._tracklimit_extactor = TrackLimitPerception(cfg)
    ...
    def run(self):
        ...
        """
        self._segmentor._setup_segmentation_model()
    ...
    def _segment_drivable_area(self) -> np.array:
        ...
        mask, vis = self._segmentor.segment_drivable_area(image)
        self._shared_mask_buffer.image = mask
        self._shared_visualisation_buffer.image = vis
        return mask

    def _extract_tracklimits(self, mask: np.array):
        tracks = self._tracklimit_extactor(mask)
