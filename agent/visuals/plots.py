...
def draw_control_map(agent, canvas: np.array, obs: Dict) -> np.array:
    try:
        ...
        utils.draw_track_lines_on_bev(
            canvas, 4, [agent.original_centre_track.T], colour=(255, 0, 255)
        )
        logger.info("Right track In plot")
        utils.draw_track_lines_on_bev(
            canvas, 4, [agent.right_track_detections], colour=(255, 255, 255)
        )
        ...
        utils.draw_track_lines_on_bev(
            canvas, 4, [np.stack([x, y], axis=0).T], colour=(0, 0, 255)
        )
